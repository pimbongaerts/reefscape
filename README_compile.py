...
(https://github.com/pimbongaerts/reefscape).
...
__copyright__ = 'Copyright (C) 2016-23 Pim Bongaerts'
__license__ = 'GPL'
...
PYTHON3_PATH = '/Library/Frameworks/Python.framework/Versions/3.6/bin/python3'

def get_python_help_message(filename):
    """ Capture help mesage of python script """
    if 'metashape_' in filename:
        raw_help_msg = subprocess.Popen([METASHAPE_PATH, '-r',
                                        filename, '-h'],
                                        stdout=subprocess.PIPE).communicate()[0]
        lines = raw_help_msg.decode('utf-8').split('\n\n')
        first_line = lines[0]
        trimmed_first_line = first_line[first_line.find('usage'):]
        
        help_msg = [trimmed_first_line] + lines[1:]

    else:
        raw_help_msg = subprocess.Popen(['python3', filename, '-h'],
                                        stdout=subprocess.PIPE).communicate()[0]
        help_msg = raw_help_msg.decode('utf-8').split('\n\n')
    return help_msg
...
def extract_metadata_from_script(filename):
    ...
    help_metadata = [item for item in help_msg if item]

    if len(help_metadata) > 2:
        metadata['usage'] = help_metadata[0]
        metadata['title'] = help_metadata[1]
        metadata['arguments'] = help_metadata[2:]
    else:
        metadata['usage'] = 'no usage information'
        metadata['title'] = ''
        metadata['arguments'] = ''      
...
def get_formatted_metadata(script, script_metadata):
    ...
    hyperlinks = get_example_hyperlinks(script_metadata[script]['usage'])
    if hyperlinks != '':
        metadata += '\n\n'
        metadata += hyperlinks
        metadata += '\n\n'
